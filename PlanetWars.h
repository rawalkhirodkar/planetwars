...
class PlanetWars {
 public:
  ...
  //--------------------------------------------------------------------------------
  void findPossibleStates(std::vector<PlanetWars>& possibleStates);
  int optimalTarget(int source_id);
  int evaluation_function();
  void UpdatePlanet(int planet_index, int owner, int ships_left);
  void findPossibleOp(std::vector<Command>& op, int& returnstate);
  PlanetWars fastForwardFleets();
  int Threshold(int planet_id);
  int Capturability(int planet_id);
  //--------------------------------------------------------------------------------
 private:
